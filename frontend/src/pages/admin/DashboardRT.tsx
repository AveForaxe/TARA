...
export const DashboardRT: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (statsRes.ok) {
        ...
        setStats({
          totalWarga: data.rt.totalWarga,
          eventMendatang: data.rt.eventMendatang,
          proposalPending: data.rt.proposalPending,
          laporanAktif: data.rt.laporanAktif
        });
      }
      if (logsRes.ok) {
        const logsData = await logsRes.json();
        if (Array.isArray(logsData)) {
          // Only show logs for current RT user
  ...
  return (
    <div className="dashboard-container">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 style={{ color: '#fff', fontSize: '2rem', margin: '0 0 8px 0' }}>Panel Ketua RT</h1>
          <p style={{ color: 'var(--on-surface-variant)', margin: 0 }}>Ringkasan Data Warga dan Laporan Blok</p>
        </div>
        <button 
          onClick={fetchData}
      ...
      <div style={{ 
        ...
        <div style={{ background: '#111214', padding: '24px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', color: '#3b82f6' }}>
            ...
            <span style={{ fontWeight: 600 }}>Total Warga Terdaftar</span>
          </div>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: '#fff' }}>{stats?.totalWarga || '0'}</div>
        </div>
        
        <div style={{ background: '#111214', padding: '24px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', color: '#10b981' }}>
            <span className="material-icons-round">event</span>
            <span style={{ fontWeight: 600 }}>Event Mendatang</span>
          </div>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: '#fff' }}>{stats?.eventMendatang || '0'}</div>
        </div>
        ...
        <div style={{ background: '#111214', padding: '24px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', color: '#ef4444' }}>
            ...
            <span style={{ fontWeight: 600 }}>Laporan Warga Aktif</span>
          </div>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: '#fff' }}>{stats?.laporanAktif || '0'}</div>
        </div>
      </div>
      ...
      <div className="glass-card" style={{ padding: '24px' }}>
        ...
        {logs.length === 0 ? (
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--on-surface-variant)' }}>
            Belum ada aktivitas yang Anda lakukan hari ini.
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {logs.map((log) => (
              <div key={log._id} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                <span className="material-icons-round" style={{ color: 'var(--outline)' }}>info</span>
                <div style={{ flex: 1 }}>
