...
export const DashboardKarangTaruna: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (statsRes.ok) {
        ...
        setStats({
          eventAktif: data.karang_taruna.eventAktif,
          totalPeserta: data.karang_taruna.totalPeserta,
          proposalPending: data.karang_taruna.proposalPending,
          danaKegiatan: `Rp ${data.karang_taruna.danaKegiatan.toLocaleString('id-ID')}`
        });
      }
      if (logsRes.ok) {
        const logsData = await logsRes.json();
        if (Array.isArray(logsData)) {
          // Only show logs for current Karang Taruna user
  ...
  return (
    <div className="dashboard-container">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 style={{ color: '#fff', fontSize: '2rem', margin: '0 0 8px 0' }}>Dashboard Karang Taruna</h1>
          <p style={{ color: 'var(--on-surface-variant)', margin: 0 }}>Manajemen Event & Kegiatan Pemuda</p>
        </div>
        <button 
          onClick={fetchData}
      ...
      <div style={{ 
        ...
        <div style={{ background: '#111214', padding: '24px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', color: '#10b981' }}>
            ...
            <span style={{ fontWeight: 600 }}>Event Aktif</span>
          </div>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: '#fff' }}>{stats?.eventAktif || '0'}</div>
        </div>
        
        <div style={{ background: '#111214', padding: '24px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', color: '#3b82f6' }}>
            <span className="material-icons-round">groups</span>
            <span style={{ fontWeight: 600 }}>Total Peserta Terdaftar</span>
          </div>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: '#fff' }}>{stats?.totalPeserta || '0'}</div>
        </div>
        ...
        <div style={{ background: '#111214', padding: '24px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', color: '#f59e0b' }}>
            ...
            <span style={{ fontWeight: 600 }}>Proposal Menunggu</span>
          </div>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: '#fff' }}>{stats?.proposalPending || '0'}</div>
        </div>

        <div style={{ background: '#111214', padding: '24px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', color: '#8b5cf6' }}>
            <span className="material-icons-round">account_balance_wallet</span>
            <span style={{ fontWeight: 600 }}>Alokasi Dana Kegiatan</span>
          </div>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: '#fff' }}>{stats?.danaKegiatan || '-'}</div>
        </div>
      </div>
      ...
      <div className="glass-card" style={{ padding: '24px' }}>
        <h3 style={{ color: '#fff', fontSize: '1.2rem', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          Log Aktivitas Event & Kegiatan
        </h3>
        {logs.length === 0 ? (
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--on-surface-variant)' }}>
            Belum ada aktivitas yang Anda lakukan.
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {logs.map((log) => (
              <div key={log._id} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                <span className="material-icons-round" style={{ color: 'var(--outline)' }}>event_note</span>
                <div style={{ flex: 1 }}>
